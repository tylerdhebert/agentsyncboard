...
export function Sidebar() {
  ...
  return (
    ...
    >
      <div className="flex h-10 flex-shrink-0 items-center gap-2 border-b border-[var(--border)] px-3">
        <span
          className={`h-1.5 w-1.5 flex-shrink-0 rounded-full ${wsConnected ? 'bg-emerald-400' : 'bg-rose-500'}`}
        />
        <span className="flex-1 text-[13px] font-semibold text-[var(--ink)]">agentsyncboard</span>
        <button
        ...
        <button
          ...
          title="New job"
          className="flex h-6 w-6 items-center justify-center rounded text-[var(--dim)] text-lg leading-none transition hover:bg-white/6 hover:text-[var(--ink)]"
        >
          +
        </button>
        <button
